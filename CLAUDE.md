...
|---|---|---|
| 执行者 | 每任务一个新子代理 | 本会话亲自 |
| 适用 | 要每任务审查门；或计划长到后续任务会在压缩后的上下文里跑；且任务数足够多（≥5）使隔离收益超过启动开销 | 任务基本独立、不需要每任务审查；计划够细；或任务数少（≤3–4） |
...
**第三个判据——固定开销 × 任务数。** 每次派发子代理都要重付一遍基础开销（system prompt + 工具定义 + 指令文件，量级数十 k token；Prompt Caching 命中时约 10%，但不保证）。任务少时该成本占单次的大头——3 个任务派 3 次 ≈ 付 3 份启动费，Native 只付 1 份（末尾审查）。**任务越多、越接近「压缩后还要接着跑」，SDD 的隔离收益才越可能抵过这笔启动费。**
