# 创建日志：系统化调试技能

提取、构建和加固关键技能的参考示例。

## 源材料

从 `~/.claude/CLAUDE.md` 提取的调试框架：
- 4 阶段系统化流程（调查 → 模式分析 → 假设 → 实施）
- 核心要求：始终找到根本原因，绝不修复症状
- 设计用于抵御时间压力和合理化行为的规则

## 提取决策

**包含内容：**
- 完整的 4 阶段框架及所有规则
- 反捷径（"绝不修复症状"、"停止并重新分析"）
- 抗压力语言（"即使更快"、"即使我看起来很匆忙"）
- 每个阶段的具体步骤

**排除内容：**
- 项目特定上下文
- 同一规则的重复变体
- 叙述性解释（精简为原则）

## 结构遵循 skill-creation/SKILL.md

1. **丰富的 when_to_use** - 包含症状和反模式
2. **类型：技术（technique）** - 带有步骤的具体流程
3. **关键词** - "根本原因"、"症状"、"变通方法"、"调试"、"调查"
4. **流程图** - "修复失败"的决策点 → 重新分析 vs 增加更多修复
5. **逐阶段分解** - 可扫描的清单格式
6. **反模式章节** - 不该做什么（对此技能至关重要）

## 加固元素

设计用于抵御压力下合理化行为的框架：

### 语言选择
- "始终（ALWAYS）" / "绝不（NEVER）"（而非"应该"/"尝试"）
- "即使更快" / "即使我看起来很匆忙"
- "停止并重新分析"（显式暂停）
- "不要跳过"（捕捉实际行为）

### 结构防御
- **阶段 1 是必需的** - 无法跳到实施
- **单一假设规则** - 强制思考，防止散弹式修复
- **显式失败模式** - "如果你的第一次修复不起作用"配强制行动
- **反模式章节** - 准确展示捷径的样子

### 冗余
- 根本原因要求出现在概览 + when_to_use + 阶段 1 + 实施规则中
- "绝不修复症状"在 4 个不同上下文中出现
- 每个阶段都有显式的"不要跳过"指导

## 测试方法

按照 skills/meta/testing-skills-with-subagents 创建了 4 个验证测试：

### 测试 1：学术场景（无压力）
- 简单 bug，无时间压力
- **结果：** 完全合规，完整调查

### 测试 2：时间压力 + 明显的快速修复
- 用户"很匆忙"，症状修复看起来很容易
- **结果：** 抵制捷径，遵循完整流程，找到真正根本原因

### 测试 3：复杂系统 + 不确定性
- 多层故障，不清楚是否能找到根本原因
- **结果：** 系统化调查，追溯所有层级，找到源头

### 测试 4：第一次修复失败
- 假设不起作用，忍不住增加更多修复
- **结果：** 停止，重新分析，形成新假设（无散弹式修复）

**所有测试通过。** 未发现合理化行为。

## 迭代

### 初始版本
- 完整的 4 阶段框架
- 反模式章节
- "修复失败"决策的流程图

### 增强 1：TDD 参考
- 添加到 skills/testing/test-driven-development 的链接
- 说明 TDD 的"最简代码"≠ 调试的"根本原因"
- 防止方法论之间的混淆

## 最终成果

加固的技能，它：
- ✅ 明确要求根本原因调查
- ✅ 抵制时间压力合理化
- ✅ 为每个阶段提供具体步骤
- ✅ 显式展示反模式
- ✅ 在多种压力场景下测试
- ✅ 阐明与 TDD 的关系
- ✅ 可投入使用

## 关键洞察

**最重要的加固：** 反模式章节展示在当下感觉合理的具体捷径。当 Claude 思考"我就加这一个快速修复"时，看到那个确切模式被列为错误会产生认知摩擦。

## 使用示例

遇到 bug 时：
1. 加载技能：skills/debugging/systematic-debugging
2. 阅读概览（10 秒） - 想起要求
3. 遵循阶段 1 清单 - 强制调查
4. 如果想跳过 - 看到反模式，停止
5. 完成所有阶段 - 找到根本原因

**时间投入：** 5-10 分钟
**节省时间：** 数小时的症状打地鼠

---

*创建时间：2025-10-03*
*目的：技能提取和加固的参考示例*
