...
| **「固定开销 × 任务数」判据** | 官方选执行路径只有两条理由（要不要每任务审查门、计划是否会被压缩）。Lite 补第三条：每次派发子代理都要重付数十 k token 的启动开销（system prompt + 工具定义 + 指令文件），任务少时该成本占大头 → 判据从「任务多就走 SDD」变成「任务多到隔离收益抵得过启动费才走 SDD」，并把「任务别默认拆越细越好」写入任务粒度 |
