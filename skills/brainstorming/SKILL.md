...
图表不是文档的装饰品，而是设计方案的组成部分。设计方案呈现给用户时就必须有图——用户看图理解架构/流程/状态，而不是靠想象。批准后才补图意味着用户批准了一个自己没看清的设计。先画图，再要批准。
...
5. **图表化呈现设计方案** — 设计方案必须包含图表，按复杂度分段呈现，每段获取用户批准（详见下方"呈现设计：必须带图"）
6. **图表完整性门控** — 设计方案获批后，校验是否缺少项目类型对应的必备图表。缺图 → 补充 → 重新校验。不缺图才能进入下一步（详见下方"图表完整性门控"）
7. **编写设计文档** — 将文字方案 + 全部图表写入 `docs/superpowers/specs/YYYY-MM-DD-<主题>-design.md` 并提交
8. **派发设计文档审查子代理** — 派独立子代理审查设计文档（详见下方"设计之后"）
9. **设计文档自审** — 审查通过后，Controller 内联检查占位符、矛盾、歧义、图表可渲染（详见下文）
10. **用户审阅书面设计文档** — 在继续之前请用户审阅设计文档文件
11. **过渡到实现** — 调用 writing-plans 技能创建实施计划
...
digraph brainstorming {
    ...
    "提供可视化伴侣\n(独立消息，不含其他内容)" -> "逐个提问澄清";
    "逐个提问澄清" -> "提出 2-3 种方案";
    "提出 2-3 种方案" -> "图表化呈现设计方案\n(文字+图表一起给用户看)";
    "图表化呈现设计方案\n(文字+图表一起给用户看)" -> "用户批准设计？";
    "用户批准设计？" -> "图表化呈现设计方案\n(文字+图表一起给用户看)" [label="否，修改"];
    "用户批准设计？" -> "图表完整性门控\n缺图？" [label="是"];
    "图表完整性门控\n缺图？" -> "补充缺失图表" [label="是"];
    "补充缺失图表" -> "图表完整性门控\n缺图？";
    "图表完整性门控\n缺图？" -> "编写设计文档" [label="否"];
...
- **设计方案必须以图表形式呈现，文字与图表一起给用户看，不得批准后再补**
- 每段的详细程度匹配其复杂度：简单的用几句话，复杂的可以到 200-300 字
- 每段之后询问是否看起来没问题
- 覆盖：架构、组件、数据流、错误处理、测试
- 每个维度必须有对应图表：架构 → ASCII 架构图，组件 → ASCII 组件树，数据流 → Mermaid flowchart/sequenceDiagram，状态变化 → Mermaid stateDiagram
- 如果有不清楚的地方，随时准备回去澄清
- **禁止等"写文档时再补图"——图是设计的一部分，不是文档的装饰**
...
设计文档必须含图。设计方案获批准后、写文档前，根据项目类型生成全部必备图表。

| 项目类型 | 必备图表 |
|---------|---------|
| 🖥️ UI | ASCII 布局原型 + Mermaid flowchart + Mermaid stateDiagram |
| ⚙️ 后端/API | ASCII 架构图 + Mermaid sequenceDiagram + Mermaid erDiagram |
| 🔗 全栈 | ASCII 布局 + ASCII 架构 + Mermaid flowchart + Mermaid sequenceDiagram |
...
设计方案获批后、编写设计文档前，强制执行图表完整性校验：

1. **对照项目类型检查必备图表** — 根据上方"图表驱动设计"中的项目类型对照表，逐一核对
2. **缺图 → 硬阻断** — 缺少任何必备图表，禁止进入"编写设计文档"步骤
3. **补充 → 重新校验** — 补齐缺失图表后，重新执行完整性校验，直到全部通过
4. **通过 → 继续** — 所有必备图表齐全，进入编写设计文档步骤

此门控不可跳过。无论项目多简单，至少一张架构/流程类图表。
...
设计文档提交后，派发独立子代理审查。使用 `skills/brainstorming/spec-document-reviewer-prompt.md` 模板构建 prompt。

审查通过后才能进入自审阶段。如果子代理发现问题，修复文档后重新提交并重新审查，直到通过。
