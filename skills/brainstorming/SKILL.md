...
6. **编写设计文档** — 将设计方案 + Mermaid 正式图表写入 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` 并提交
7. **派发设计文档审查子代理** — 派独立子代理审查设计文档（详见下方"设计之后"）
8. **设计文档自审** — 审查通过后，Controller 内联检查占位符、矛盾、歧义、图表完整性（见下文）
...
2. **内部一致性：** 是否有章节之间存在矛盾？架构是否与功能描述匹配？Mermaid 图是否覆盖了文字描述的所有路径？
3. **范围检查：** 是否足够聚焦以放入一个实施计划，还是需要拆解？
4. **歧义检查：** 是否有任何需求可以有两种不同的解释？如果有，选择一种并明确说明。
5. **图表完整性：** 讨论中确定的 ASCII 概念是否都已转化为 Mermaid 正式图表？Mermaid 语法是否正确？
